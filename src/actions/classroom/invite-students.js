...
export async function inviteStudents(classroomId, studentEmails) {
  try {
    ...
    const classroom = await ClassroomModel.findOne({
      _id: classroomId,
      teacher: teacher._id,
    }).select('classroomName classroomCode students');
    ...
    for (const email of studentEmails) {
      try {
        ...
        if (studentUser) {
          ...
          const student = await StudentModel.findOne({ userId: studentUser._id });
          
          if (student) {
            // Check if student is already in this classroom
            // Check both classroom.students array and student.classrooms array
            ...
            if (isAlreadyMember) {
              results.push({
                email,
                success: false,
                message: 'Student is already a member of this classroom',
              });
              continue; // Skip sending invite
            }
          }
        }
